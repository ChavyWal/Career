...
Phonenumber char (12) not null constraint ck_Phone_number_cannot_be_blank check(phonenumber <> '')
...
CurrentOccupationCluster VARCHAR (50) null    
    ...
    constraint ck_Date_of_birth_should_be_at_least_18_years_before_Date_first_consultation check(datediff(year, Dateofbirth, DateFoundJob) >= 18),
    constraint ck_score1_cannot_be_lower_than_score2 check(score1 >= score2 ),
    constraint ck_score2_cannot_be_lower_than_score3 check (score2 >= score3),
    constraint ck_cluster2_cannot_be_the_same_as_cluster1_or_3 check(cluster2 not in (cluster1, cluster3)),
    constraint ck_cluster1_cannot_be_the_same_as_cluster2_or_3 check(cluster1 not in (cluster2, cluster3)),
